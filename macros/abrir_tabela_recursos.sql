{% macro abrir_tabela_recursos(date_range_start, date_range_end, is_incremental) %}
...
WITH recurso_sppo AS (
  ...
  {% if is_incremental -%}
    WHERE
        DATE(data) BETWEEN DATE("{{date_range_start}}") 
        AND DATE("{{date_range_end}}")
        AND DATETIME(PARSE_TIMESTAMP('%Y-%m-%d %H:%M:%S%Ez', timestamp_captura), 
        "America/Sao_Paulo") BETWEEN DATETIME("{{date_range_start}}") 
