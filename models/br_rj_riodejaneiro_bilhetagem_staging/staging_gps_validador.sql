...
SELECT
    ...
    SAFE_CAST(JSON_VALUE(content, '$.bytes_recebidos_app') AS INTEGER) AS bytes_recebidos_app,
    SAFE_CAST(JSON_VALUE(content, '$.bytes_recebidos_geral') AS INTEGER) AS bytes_recebidos_geral,
    SAFE_CAST(JSON_VALUE(content, '$.bytes_transmitidos_app') AS INTEGER) AS bytes_transmitidos_app,
    SAFE_CAST(JSON_VALUE(content, '$.bytes_transmitidos_geral') AS INTEGER) AS bytes_transmitidos_geral,
    ...
    SAFE_CAST(JSON_VALUE(content, '$.qtd_transacoes_enviadas') AS INTEGER) AS qtd_transacoes_enviadas,
    SAFE_CAST(JSON_VALUE(content, '$.qtd_transacoes_pendentes') AS INTEGER) AS qtd_transacoes_pendentes,
    SAFE_CAST(JSON_VALUE(content, '$.qtd_venda_botao') AS INTEGER) AS qtd_venda_botao,
