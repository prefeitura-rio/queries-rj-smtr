...
-- TODO: Apenas view funciona. Faz sentido ter essa tabela no datalake? Pensar em outra forma
...
SELECT
  ...
  CASE
    ...
    WHEN "{{ var('run_date') }}" >= "2024-03-01" AND "{{ var('run_date') }}" < "2024-03-16" THEN DATE("2024-03-25")
    -- Versão fixa do STU em 2024-04-09 para mar/Q2 devido à falha de atualização na fonte da dados (SIURB)
    WHEN "{{ var('run_date') }}" >= "2024-03-16" AND "{{ var('run_date') }}" < "2024-04-01" THEN DATE("2024-04-09")
    ELSE (
      SELECT MIN(data)
      ...
      WHERE data >= DATE_ADD(DATE("{{ var('run_date') }}"), INTERVAL 5 DAY)
        ...
        AND ("{{ var('run_date') }}" < "2024-04-01" OR data >= '2024-04-09')
    )
