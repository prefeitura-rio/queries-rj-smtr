...
WITH
  ...
  FROM
    {{ ref("sumario_servico_tipo_viagem_dia") }} -- `rj-smtr`.`dashboard_subsidio_sppo`.`sumario_servico_tipo_viagem_dia`
  WHERE
    DATA BETWEEN DATE('2022-06-01')
    AND DATE('2023-11-30')
    AND DATA NOT IN ('2022-10-02',
  ...
  WHERE
    ...
    AND data_inicio <= DATE('2023-11-30')
    -- Remove quilometragem irregular
    AND status NOT IN ("Não licenciado", "Nao licenciado")
...
FROM
  receita_esperada AS re
JOIN
  receita_aferida AS ra
ON
  re.data = ra.data
  AND re.servico = ra.servico