...
set start_date = "2021-08-24" % } { % endif % } with trips as (
              SELECT trip_id,
                     shape_id,
                     route_id,
       ...
       ids as (
              SELECT trip_id,
                     ...
                     m.data_versao,
                     row_number() over(
                            partition by m.data_versao,
                            m.shape_id,
                            l.trip_id
                     ) rn
              FROM merged m
                     JOIN linhas l ON m.shape_id = l.shape_id
                     AND m.data_versao = l.data_versao -- mudar join para o route_id em todas as dependencias
       )
SELECT *
except(rn)
FROM ids
WHERE rn = 1 { % if is_incremental % }
       AND data_versao > DATE("{{start_date}}") { % endif % }